...
export function DaysAmountInThePanel(year: number, month: number): Panel {
  ...
  const lastDayInTheCurrentMonths: Date = new Date(
    year,
    month,
    amountDaysInTheMonths[month]
  );

  const TheWeekDayOfTheFirstMonthsDay: number =
    firstDayInTheCurrentMonths.getDay();

  const TheWeekDayOfTheLastMonthsDay: number =
    lastDayInTheCurrentMonths.getDay();

  let firstDayInThePanel: number;

  if (month === 0 && TheWeekDayOfTheFirstMonthsDay === 0) {
    firstDayInThePanel =
      new Date(year - 1, 11, amountDaysInTheMonths[11]).getDate() +
      TheWeekDayOfTheFirstMonthsDay -
      5;
  } else if (TheWeekDayOfTheFirstMonthsDay === 0) {
    firstDayInThePanel =
      new Date(year, month - 1, amountDaysInTheMonths[month - 1]).getDate() +
      TheWeekDayOfTheFirstMonthsDay -
      5;
  } else if (month === 0) {
    firstDayInThePanel =
  ...
  const lastDayInThePanel: number = new Date(
    year,
    month + 1,
    7 - TheWeekDayOfTheLastMonthsDay
  ).getDate();

  if (month === 0) {
    for (
      let index: number = amountDaysInTheMonths[11];
    ...
    for (
      let index: number = amountDaysInTheMonths[month - 1];
      index >= firstDayInThePanel;
      index--
    ) {
      daysAmountInThePreviousMonths.unshift(index);
