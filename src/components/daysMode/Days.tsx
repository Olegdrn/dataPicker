...
export const Days: React.FC = () => {
  ...
  const putDaysInCurrent =(value:number)=>{
    dispatch(dateChanging(new Date(currentDate.getFullYear(), currentDate.getMonth(), value)))
  }
  ...
  return (
    ...
    <div className={styles.calendar}>
      ...
      <p className={styles.dayCellBefAft} key={index} onClick={()=>{putDaysInPrevious(value)}}>
        {value}
      </p>
      )}
      {daysAmountInTheCurrentMonths.map((value:number, index:number)=>
      <p className={styles.dayCell} key={index} onClick={()=>{putDaysInCurrent(value)}
      }>
        {value}
      </p>
      )}
      {daysAmountInTheNextMonths.map((value:number, index:number)=>
      <p className={styles.dayCellBefAft} key={index} onClick={()=>{putDaysInNext(value)}}>
        {value}
        </p>
      )}
    </div>
